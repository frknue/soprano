...
The session scratchpad (`/tmp/soprano-scratch`) contains `driver` (postToPid event injector, source `driver.swift`) and `allwin` (window lister, source `allwin.swift`). If the binaries are missing, recompile:
