...
The scratchpad is `/tmp/soprano-scratch` (call it `$SCRATCH`). It starts empty; Task 2 Step 1 creates the two helper tools. `driver` posts key events to a pid (needs Accessibility permission for the invoking terminal); `allwin` lists a pid's windows (window names need Screen Recording permission).
...
pkill -x Soprano; sleep 1
~/git/soprano/.build/debug/Soprano > /dev/null 2>&1 &
sleep 3
