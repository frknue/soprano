...
- **Do NOT launch the GUI (`.build/debug/Soprano`, `swift run`, `./run.sh`) during verification** — it spawns login shells that trigger repeated macOS "Ghostty would like to access data from other apps" prompts (see CLAUDE.md). Verify the logic with a standalone `swiftc` harness (Step 6); the visual checks are handed to the user as a manual checklist (Step 7). `$SCRATCH` = `/tmp/soprano-scratch`.
