...
- Verification helpers `driver`/`allwin` exist in `$SCRATCH` = `/tmp/soprano-scratch`. Key codes: a=0, s=1, m=46, l=37. Kill instances with `pkill -x Soprano` between launches.
